...
function fenceDelimiter(cleaned: string): FenceDelimiter | undefined {
...
function renderCodeBlock(language: string, lines: readonly string[]): string {
  const longestRun = lines.reduce(
    (longest, line) =>
      [...line.matchAll(/`+/gu)].reduce(
        (lineLongest, run) => Math.max(lineLongest, run[0].length),
        longest,
      ),
    0,
  );
...
function findCodeRegions(
  ...
  let opened: { index: number; language: string; marker: string } | undefined;
...
function rewriteParagraphRun(
  run: readonly Paragraph[],
  source: string,
  warnings: Set<string>,
): RewrittenRun | undefined {
  ...
  const regionsByStart = new Map(
    regions.map((region) => [region.start, region]),
  );
  const blocks: string[] = [];
  ...
  while (index < lines.length) {
    ...
    if (region) {
      ...
      blocks.push(
        renderCodeBlock(
          region.language,
          lines.slice(region.start + 1, region.end).map((line) => line.cleaned),
        ),
      );
      index = region.end + 1;
      continue;
 ...
 */
export function restoreCodeFences(source: string): RestoredCode {
  const tree = blockProcessor.parse(source) as Root;
...
function splitInlineCode(value: string): Nodes[] | undefined {
  const nodes: Nodes[] = [];
  let cursor = 0;
 ...
 */
export function restoreInlineCode(tree: Root): void {
  const visit = (node: Parents): void => {
    ...
    for (const child of node.children) {
      if (child.type === 'text') {
        ...
        if (split) {
          children.push(...(split as RootContent[]));
          changed = true;
          continue;
        }
