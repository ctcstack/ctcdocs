...
export function selectFailureStage(
  environment: Record<string, string | undefined>,
): string {
  return (
    STAGE_VARIABLES.find(
      ([, variable]) => environment[variable] === 'failure',
    )?.[0] ?? 'workflow'
  );
}
...
export async function notifySyncFailure(
  ...
  if (!webhook) {
    console.log(
      'Sync failure webhook is not configured; notification skipped.',
    );
    return false;
  }
