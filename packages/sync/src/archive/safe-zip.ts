...
export class UnsafeZipError extends Error {
  override readonly name = 'UnsafeZipError';
}
...
function readEntry(
  ...
  const localCompressionMethod = archive.readUInt16LE(
  ...
  const localPath = normalizeArchivePath(
    archive
      .subarray(
        entry.localHeaderOffset + 30,
        entry.localHeaderOffset + 30 + fileNameLength,
      )
      .toString('utf8'),
  );
  ...
  if (
    ...
    throw new UnsafeZipError(
      'ZIP local-file data is inconsistent with its central directory.',
    );
  }
  ...
  if (
    data.byteLength !== entry.uncompressedBytes ||
...
export function extractSafeZipEntries(input: Uint8Array): ExtractedZipEntry[] {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    const entryLength =
      46 + fileNameLength + extraFieldLength + entryCommentLength;

    if (offset + entryLength > endOffset) {
      throw new UnsafeZipError(
        'ZIP central-directory entry exceeds archive bounds.',
      );
    }
    if (
    ...
    if (localHeaderOffset >= centralDirectoryOffset) {
      throw new UnsafeZipError(
        'ZIP local-file header overlaps the central directory.',
      );
    }

    const rawPathBytes = archive.subarray(
      offset + 46,
