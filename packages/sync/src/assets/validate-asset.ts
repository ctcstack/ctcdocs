...
export class UnsafeAssetError extends Error {
  override readonly name = 'UnsafeAssetError';
}
...
function sortElementAttributes($: ReturnType<typeof cheerio.load>): void {
  $('*').each((_, element) => {
...
function validateSvg(bytes: Uint8Array): Uint8Array {
  let source: string;
  try {
