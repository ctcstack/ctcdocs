 ...
 */
const generatedAssetPattern = new RegExp(
  `^${PROJECT_LAYOUT.generatedAssetsDirectory.replaceAll('/', '\\/')}\\/[A-Za-z0-9_-]+\\/image-\\d{3}\\.(?:gif|jpg|png|svg|webp)$`,
  'u',
);
...
export class GeneratedOutputValidationError extends Error {
  override readonly name = 'GeneratedOutputValidationError';
}

function parseFrontmatter(content: string, markdownHeader: string): unknown {
...
async function collectAssetFiles(
  ...
  for (const entry of entries.sort((left, right) =>
    ...
    if (entry.isDirectory()) {
      for (const [nestedPath, bytes] of await collectAssetFiles(
        assetRoot,
        path,
      )) {
      ...
      files.set(
        `${PROJECT_LAYOUT.generatedAssetsDirectory}/${relative(assetRoot, path).replaceAll('\\', '/')}`,
        await readFile(path),
      );
    } else {
...
async function validateGeneratedOutputInternal(
  stagedRepositoryRoot: string,
  context: SyncContext,
): Promise<void> {
  ...
  const manifest = syncManifestSchema.parse(
    JSON.parse(
      await readFile(
        resolve(stagedRepositoryRoot, PROJECT_LAYOUT.manifestFile),
        'utf8',
      ),
    ),
  );
  const aiIndex = aiDocsIndexSchema.parse(
    JSON.parse(
      await readFile(
        resolve(stagedRepositoryRoot, 'data/docs-index.json'),
        'utf8',
      ),
    ),
  );
  syncReportSchema.parse(
    JSON.parse(
      await readFile(
        resolve(stagedRepositoryRoot, 'data/latest-sync-report.json'),
        'utf8',
      ),
    ),
  );
  ...
  if (
    JSON.stringify(generatedFileNames) !== JSON.stringify(expectedFileNames)
  ) {
    throw new Error(
      'Generated Markdown files do not exactly match the sync manifest.',
    );
  }
  ...
  const linkDocuments: Array<{ body: string; stableSlug: string }> = [];
  for (const record of Object.values(manifest.documents)) {
    ...
    const content = await readFile(
      resolve(stagedRepositoryRoot, expectedPath),
      'utf8',
    );
    ...
    const documentAssets = new Map<string, Uint8Array>();
    for (const imageUrl of collectMarkdownImageUrls(body)) {
      const repositoryPath = posix.normalize(
        posix.join(posix.dirname(expectedPath), imageUrl),
      );
      if (
        !repositoryPath.startsWith(`${expectedAssetsDirectory}/`) ||
    ...
    if (
      computeGeneratedContentHash(
        body,
        [...documentAssets].map(([repositoryPath, bytes]) => ({
          repositoryPath,
          bytes,
        })),
      ) !== record.contentHash
      ...
      throw new Error(
        'Generated Markdown and assets do not match the content hash.',
      );
    }
  ...
  for (const folder of Object.values(manifest.folders)) {
    ...
    const content = await readFile(
      resolve(stagedRepositoryRoot, folder.generatedMarkdownPath),
      'utf8',
    );
    const frontmatter = sectionFrontmatterSchema.parse(
      parseFrontmatter(content, markdownHeader),
    );
    if (
    ...
    if (
      computeGeneratedContentHash(body.trimEnd(), []) !==
  ...
  if (
    [...allAssets.keys()].some(
      (repositoryPath) => !referencedAssets.has(repositoryPath),
    )
  ) {
  ...
  if (
    aiIndex.documents.length !== Object.keys(manifest.documents).length ||
    aiIndex.documents.some(
      (entry) =>
        manifest.documents[entry.id]?.generatedMarkdownPath !==
        entry.markdownPath,
    )
  ) {
  ...
  const sidebar = await readFile(
    resolve(
      stagedRepositoryRoot,
      `${PROJECT_LAYOUT.generatedSourceDirectory}/sidebar.ts`,
    ),
    'utf8',
  );
  ...
  const redirects = await readFile(
    resolve(
      stagedRepositoryRoot,
      `${PROJECT_LAYOUT.generatedSourceDirectory}/redirects.ts`,
    ),
    'utf8',
  );
  if (
...
export async function validateGeneratedOutput(
  ...
  try {
    ...
    throw new GeneratedOutputValidationError(
