...
import { GENERATED_DIRECTORY_ALLOWLIST, isGeneratedPathAllowed, PROJECT_LAYOUT, } from '@ctcstack/ctcdocs-core';
import { parse as parseJsonWithComments } from 'jsonc-parser';
import { parse as parseToml } from 'smol-toml';
import { z } from 'zod';
const runCommand = promisify(execFile);
function protectedEnvironmentSchema(pattern) {
    return z
        .object({
 ...
 */
function wranglerConfigurationSchema(site) {
    const { deployment } = site;
    const environments = Object.fromEntries(Object.entries(deployment.environments).map(([name, environment]) => [
        name,
...
function gitleaksExemptPatterns(content) {
 ...
 */
async function validateSecretScanExemptions(repositoryRoot) {
    const configurationPath = resolve(repositoryRoot, PROJECT_LAYOUT.gitleaksConfigurationFile);
    ...
    const patterns = [];
    const errors = [];
    for (const pattern of gitleaksExemptPatterns(await readFile(configurationPath, 'utf8'))) {
        try {
            patterns.push(new RegExp(pattern));
        }
        catch {
            errors.push(`${PROJECT_LAYOUT.gitleaksConfigurationFile} exempts a path pattern that cannot be interpreted: ${pattern}`);
        }
    }
    ...
    for (const path of tracked) {
        if (patterns.some((pattern) => pattern.test(path))) {
            errors.push(`Git tracks ${path}, which the secret scanner is configured to ignore. Remove it from the index and rotate anything it exposed.`);
        }
    }
    return errors;
}
export async function validateRepositoryContent(context) {
    ...
    checkedFiles += 1;
    try {
        wranglerConfigurationSchema(site).parse(parseJsonWithComments(wranglerContent));
    }
    catch {
        const names = Object.keys(site.deployment.environments).sort().join(', ');
        errors.push(`${PROJECT_LAYOUT.wranglerConfigurationFile} must deploy Worker ${site.deployment.workerName} to exactly these environments and their configured hostnames, with public Worker URLs disabled: ${names}`);
    }
    errors.push(...(await validateSecretScanExemptions(repositoryRoot)));
    checkedFiles += 1;
    const hasCorpus = await pathExists(resolve(repositoryRoot, PROJECT_LAYOUT.manifestFile));
    for (const allowedDirectory of GENERATED_DIRECTORY_ALLOWLIST) {
        ...
        for (const absolutePath of await listFilesRecursively(absoluteDirectory)) {
            ...
            if (!isGeneratedPathAllowed(repositoryPath)) {
                errors.push(`Generated file is outside the allowlist: ${repositoryPath}`);
                continue;
            }
            if (allowedDirectory === PROJECT_LAYOUT.generatedDocumentsDirectory &&
            ...
            if (repositoryPath.endsWith('.md')) {
                ...
                if (!content.includes(markdownHeader)) {
                    errors.push(`Generated Markdown has no ownership header: ${repositoryPath}`);
                }
            }
        }
    }
