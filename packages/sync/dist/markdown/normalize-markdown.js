...
export class MarkdownNormalizationError extends Error {
    issues;
    name = 'MarkdownNormalizationError';
    constructor(issues) {
        super(`Markdown normalization failed (${issues.length} issue${issues.length === 1 ? '' : 's'}).`);
        this.issues = issues;
    }
}
...
function normalizedComparisonText(node) {
...
function truncateDescription(value, maximumLength = 200) {
    ...
    const truncated = lastSpace >= Math.floor(maximumLength * 0.6)
...
function isGeneratedAssetUrl(value) {
    return /^(\.\.\/){3}assets\/generated\/[A-Za-z0-9_-]+\/image-\d{3}\.(?:gif|jpg|png|svg|webp)$/u.test(value);
}
...
function validateNodes(tree, options) {
    const issues = [];
    walk(tree, (node) => {
        ...
        else if ((node.type === 'link' && !isSafeUrl(node.url)) ||
            (node.type === 'definition' && !isSafeUrl(node.url))) {
            issues.push({ code: 'unsafe_url' });
        }
    });
    return issues;
}
function normalizeHeadings(tree, title) {
    const firstContentIndex = tree.children.findIndex((node) => node.type !== 'thematicBreak');
    const firstContent = tree.children[firstContentIndex];
    if (firstContent?.type === 'heading' &&
...
export function normalizeMarkdown(input, title, options = {}) {
    ...
    const tree = markdownProcessor.parse(restored.markdown);
