 ...
 */
export async function writeSyncSummary(repositoryRoot, environment = process.env) {
    const summaryPath = environment.GITHUB_STEP_SUMMARY;
    if (!summaryPath) {
        throw new SyncSummaryError('GITHUB_STEP_SUMMARY is not configured.');
    }
    const report = syncReportSchema.parse(JSON.parse(await readFile(resolve(repositoryRoot, PROJECT_LAYOUT.syncReportFile), 'utf8')));
    await appendFile(summaryPath, renderSyncJobSummary(report, environment.SYNC_OUTPUT_CHANGED === 'true'), 'utf8');
