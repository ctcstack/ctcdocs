 ...
 */
export function unwrapGoogleRedirect(value) {
    let wrapper;
    try {
    ...
    if (!GOOGLE_REDIRECT_HOSTS.has(wrapper.hostname) ||
        wrapper.pathname !== '/url') {
        return undefined;
    }
    const target = wrapper.searchParams.get('q') ?? wrapper.searchParams.get('url');
