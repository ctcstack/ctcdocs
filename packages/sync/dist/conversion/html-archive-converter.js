...
import { UnsafeAssetError, validateImageAsset, } from '../assets/validate-asset.js';
...
export class HtmlArchiveConversionError extends Error {
    name = 'HtmlArchiveConversionError';
}
...
function selectHtmlEntry(entries) {
    ...
    if (htmlEntries.length !== 1) {
        throw new HtmlArchiveConversionError('HTML ZIP must contain exactly one HTML document.');
    }
...
function isComplexDomTable(node) {
...
function isComplexCheerioTable($, table) {
    ...
    $(table)
        ...
        const cells = $(row)
            .children('td, th')
            .toArray()
            .filter((cell) => cell instanceof Element);
        columnCounts.add(cells.length);
        if (cells.some((cell) => {
...
function simpleTableMarkdown(node) {
...
function allowedAttributes(elementName) {
    if (elementName === 'a') {
...
export function convertHtmlArchive(entries, options) {
    ...
    $('body *').each((_, element) => {
        ...
        for (const [attributeName, value] of Object.entries(element.attribs)) {
            ...
            else if ((normalizedName === 'colspan' || normalizedName === 'rowspan') &&
    ...
    $('body img').each((_, element) => {
        ...
        if (!archiveAsset) {
            throw new HtmlArchiveConversionError('HTML references a missing archive asset.');
        }
        let validated;
        try {
    ...
    const hasComplexTables = $('body table')
        .toArray()
        .some((table) => table instanceof Element && isComplexCheerioTable($, table));
