...
const DOCUMENT_FIELDS = 'tabs(tabProperties(tabId),documentTab(inlineObjects,positionedObjects),childTabs(tabProperties(tabId)))';
...
export class GoogleDocsClient {
    ...
    async inspectDocument(fileId) {
        const url = new URL(`${this.baseUrl}/documents/${encodeURIComponent(fileId)}`);
        ...
        try {
            ...
            throw new GoogleApiError('Google Docs API returned an invalid structural response.', 'invalid_response', response.status, this.safeRequestId(response), { cause: error });
        }
        if (parsed.tabs.some((tab) => (tab.childTabs?.length ?? 0) > 0)) {
            throw new GoogleApiError('Nested Google Docs tabs are not supported by this converter version.', 'invalid_response', response.status, this.safeRequestId(response));
        }
        ...
        for (const tab of parsed.tabs) {
            ...
            const positionedObjects = Object.values(tab.documentTab?.positionedObjects ?? {});
            inlineObjectCount += inlineObjects.length;
            positionedObjectCount += positionedObjects.length;
            for (const embeddedObject of [
                ...inlineObjects.map((object) => object.inlineObjectProperties?.embeddedObject),
                ...positionedObjects.map((object) => object.positionedObjectProperties?.embeddedObject),
            ]) {
                hasEmbeddedDrawings ||= Boolean(embeddedObject?.embeddedDrawingProperties);
    ...
    async request(url) {
        for (let attempt = 0; attempt <= this.options.maxRetries; attempt += 1) {
            ...
            try {
                ...
                throw new GoogleApiError('Google Docs API network request failed.', 'network', undefined, 'unavailable', { cause: error });
            }
            if (!response.ok) {
                ...
                throw new GoogleApiError(`Google Docs API request failed with status ${response.status}.`, categorizeGoogleApiStatus(response.status), response.status, this.safeRequestId(response));
            }
            return response;
        }
        throw new GoogleApiError('Google Docs API retry loop terminated unexpectedly.', 'invalid_response', undefined, 'unavailable');
    }
