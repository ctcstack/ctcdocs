...
import { GENERATED_DIRECTORY_ALLOWLIST, GENERATED_FILE_ALLOWLIST, } from '@ctcstack/ctcdocs-core';
...
import { SyncSummaryError, writeSyncSummary, } from './automation/write-sync-summary.js';
...
import { createEnvironmentGoogleAccessTokenProvider, GoogleAuthenticationConfigurationError, } from './google/auth.js';
...
function printSyncSummary(result, dryRun) {
...
function printInventorySummary(result) {
    const { summary } = result.report;
    ...
    for (const [code, count] of [...warningCounts].sort(([left], [right]) => left < right ? -1 : 1)) {
...
async function main() {
    ...
    if (options.command === 'validate') {
        ...
        console.log(`Content validation passed (${result.checkedFiles} files checked).`);
        if (!result.hasCorpus) {
            console.log('This project has never synchronized: there is no generated corpus to check yet.');
        }
        return;
    }
    const configuration = parseSyncConfiguration(process.env, context.site);
    const accessTokenProvider = createEnvironmentGoogleAccessTokenProvider(process.env);
    if (options.inventoryOnly) {
        const result = await runInventory(context, configuration, accessTokenProvider);
        if (options.json) {
    ...
    const result = await runBasicMarkdownSync(context, configuration, accessTokenProvider, {
        dryRun,
...
try {
    ...
    if (error instanceof GoogleAuthenticationConfigurationError) {
        ...
        const exitCode = error.category === 'authentication'
            ? 2
            : error.category === 'permission'
                ? 3
                : 1;
        console.error(`ERROR [GOOGLE_${error.category.toUpperCase()}]: status=${error.status ?? 'unavailable'} requestId=${error.requestId}`);
        process.exitCode = exitCode;
    }
    else if (error instanceof InventoryGraphError) {
        console.error(`ERROR [INVENTORY_GRAPH]: ${error.issues.map((issue) => issue.code).join(',')}`);
        process.exitCode = 1;
    }
    else if (error instanceof MarkdownNormalizationError) {
        console.error(`ERROR [MARKDOWN_NORMALIZATION]: ${error.issues.map((issue) => issue.code).join(',')}`);
        process.exitCode = 1;
    }
    else if (error instanceof UnsafeZipError ||
        ...
        console.error(`ERROR [CONFIGURATION]: ${error.issues.map((issue) => issue.path.join('.') || 'environment').join(',')}`);
        process.exitCode = 1;
    }
