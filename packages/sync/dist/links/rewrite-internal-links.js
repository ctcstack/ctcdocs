...
function parseGoogleDocumentLink(value) {
    let url;
    try {
        url = new URL(value);
    }
    ...
    if (url.hostname === 'docs.google.com') {
        const match = /^\/document\/d\/([^/]+)\/(?:edit|view)\/?$/u.exec(url.pathname);
        fileId = match?.[1] ?? null;
    }
...
export function rewriteInternalGoogleLinks(body, stableSlugs) {
