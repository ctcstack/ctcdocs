...
export function isGeneratedPathAllowed(input: string): boolean {
  ...
  if (
    GENERATED_FILE_ALLOWLIST.some((allowedPath) => normalized === allowedPath)
  ) {
    return true;
  }

  return GENERATED_DIRECTORY_ALLOWLIST.some(
    (allowedDirectory) =>
      normalized === allowedDirectory ||
      normalized.startsWith(`${allowedDirectory}/`),
  );
}

export function assertGeneratedPathAllowed(input: string): void {
