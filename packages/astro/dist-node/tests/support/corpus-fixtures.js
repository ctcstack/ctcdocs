...
function readDocuments() {
    const index = JSON.parse(readFileSync(resolve(repositoryRoot, PROJECT_LAYOUT.documentIndexFile), 'utf8'));
    const documents = typeof index === 'object' && index !== null && 'documents' in index
        ? index.documents
        : undefined;
    if (!Array.isArray(documents) || documents.length === 0) {
        throw new Error(`${PROJECT_LAYOUT.documentIndexFile} lists no documents; run a sync before the browser tests.`);
    }
    return documents;
}
 ...
 */
export function documentInFolder() {
 ...
 */
export function documentWithTable() {
    return documents.find((document) => {
        let body;
        try {
            body = readFileSync(resolve(repositoryRoot, PROJECT_LAYOUT.generatedDocumentsDirectory, `${document.id}.md`), 'utf8');
        }
        catch {
            return false;
        }
...
export function documentWithAsset() {
    for (const document of documents) {
        const directory = resolve(repositoryRoot, PROJECT_LAYOUT.generatedAssetsDirectory, document.id);
        let assets;
        try {
        ...
        const asset = assets.find((name) => statSync(resolve(directory, name)).isFile());
        if (asset) {
            return {
